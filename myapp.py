...
#title
st.title(':rainbow[Data Analytic Portal]')
...
if(file != None):
    ...
    st.dataframe(data)
    st.info('File is successfully Uploadede',icon='👍')
    ...
    with tab4:
        st.subheader(':gray[Column Namesn in Dataset]')
    ...
    with st.expander('Group by your columns'):
        ...
        if(groupby_cols):
            ...
            if(graphs=='line'):
                x_axis = st.selectbox('Choose X asix', options=list(result.columns))
                y_axis = st.selectbox('Choose Y asix', options=list(result.columns))
                ...
                fig = px.pie(data_frame=result, names=names, template='plotly_white')
                st.plotly_chart(fig)
